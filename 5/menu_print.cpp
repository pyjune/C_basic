...
void menu_print(MenuType pMenu[], int menuN);
...
int main(void) 
{
	...
	MenuType cupArr[3] = {{"Latte", 1000},{"Mocha", 2000},{}};
	
	printf("%s %d\n", cupArr[0].name, cupArr[0].price);
	strcpy(cupArr[2].name, "Ameri");
	cupArr[2].price = 800;
	for(int i=0; i<3; i++)
	{
		printf("%s %d\n", cupArr[i].name, cupArr[i].price);
	}
	menu_print(cupArr, 3);
